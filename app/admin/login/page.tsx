...
export default async function AdminLoginPage() {
  ...
  return (
    <main className="fixed inset-0 overflow-y-auto bg-[#060711] text-white selection:bg-cyan-300 selection:text-black">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(34,211,238,.14),transparent_34%),radial-gradient(circle_at_90%_80%,rgba(217,70,239,.12),transparent_36%)]" />
          ...
          <div className="border-b border-white/10 p-8 sm:p-12 lg:border-b-0 lg:border-r">
            ...
            <div className="mt-16 max-w-md">
              ...
              <div className="mt-10 grid gap-3 text-sm text-white/65 sm:grid-cols-2">
                <Feature label="Protected route" detail="Server-verified identity" />
                <Feature label="Owner access" detail="Email allowlist" />
                <Feature label="Live status" detail="Production overview" />
                <Feature label="Fast handoff" detail="Back to game anytime" />
              </div>
            </div>
          </div>
            ...
            <div className="w-full">
              ...
              {user ? (
                <div className="space-y-4">
                  ...
                  <a href={signOutPath()} className="flex w-full items-center justify-center rounded-xl border border-white/15 bg-white/[.06] px-5 py-3.5 text-sm font-bold transition hover:bg-white/10">Sign out and use another account</a>
                </div>
              ) : (
...
function Feature({ label, detail }: { label: string; detail: string }) {
  return <div className="rounded-xl border border-white/[.07] bg-black/20 p-4"><div className="font-semibold text-white/85">{label}</div><div className="mt-1 text-xs text-white/35">{detail}</div></div>;
}
